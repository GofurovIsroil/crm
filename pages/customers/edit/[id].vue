...
interface InputFieldEvent extends Event {
...
const custemerId = route.params.id as string;

const { handleSubmit, defineField, setFieldValue, setValues, values } =
  useForm<ICustomerFormState>();

const { data, isSuccess } = useQuery({
  queryKey: ["get customer", custemerId],
  queryFn: () => DB.getDocument(DB_ID, COLLECTION_CUSTOMERS, custemerId),
});
...
const { mutate, isPending } = useMutation({
  mutationKey: ["update customer", custemerId],
  mutationFn: (data: ICustomerFormState) =>
    DB.updateDocument(DB_ID, COLLECTION_CUSTOMERS, custemerId, data),
});
    ...
    <form @submit="onSubmit" class="form">
      ...
      <div class="grid w-full max-w-sm items-center gap-1.5 input">
        <label>
          ...
          <UiInput
            type="file"
            :onchange="(e:InputFieldEvent)=>e?.target?.files?.length && uploadImage(e.target.files[0])"
...
<style scoped>
.input {
  @apply border-[#161c26] mb-2 placeholder:text-[#748092] focus:border-border transition-all;
}
</style>
