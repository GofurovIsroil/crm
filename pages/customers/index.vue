...
const { data: custemers, isLoading } = useQuery({
  queryKey: ["customers"],
...
<template>
  <div class="p-10">
    ...
    <UiTable v-else>
      <UiTableHeader>
        <UiTableRow>
          <UiTableHead class="w-[80px]">Picture</UiTableHead>
          <UiTableHead class="w-[80px]">Name</UiTableHead>
          <UiTableHead class="w-[80px]">Email</UiTableHead>
          <UiTableHead class="w-[80px]">Source</UiTableHead>
        </UiTableRow>
      </UiTableHeader>
      <UiTableBody>
        <UiTableRow
          v-for="customer in (custemers?.documents as unknown as ICustomer[])"
          :key="customer.$id"
        >
